(ns sepa-ct.main
  "Kotodama WASM entrypoint for the sepa_ct clean-room actor (L4) — Clojure port.
  ...
  facts (`sepa_ct.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sepa_ct")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"sepa_ct.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sepa_ct-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sepa_ct-compat/ready)
